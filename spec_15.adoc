...
job request *_R_* into *_R'_*, and SHALL share the canonicalized signed
*_R'_* in a safe export of data to the privileged IMP parent.
...
integrity verification, or in any way modify the user request *_J_*. The
...
Request *_R'_*, it SHALL perform the following verification steps:

 1. Verify integrity and authenticity of *_R'_*
 2. Verify owner has access to assigned resource set
 3. Verify integrity and authenticity of *_J_*
 4. Verify TTL on *_R'_* and *_J_*
...
After *_R'_* verification is complete, the `flux-security-imp`
...
_prolog_ and _epilog_ for jobs. These scripts are typically site-supplied
