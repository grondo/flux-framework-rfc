...
becomes the user request token *J* which authorizes the resource owner to
...
time-to-live, become a job request token *R* which authorizes the guest
user access to the specific resources in the resource set.

This final signed job request token *R* becomes the input to the
...
The IMP verifies the integrity and authenticity of both *R* and *J*
...
passing the verfied *J* as input to the shell. The shell itself is
...
shell is invoked, and this is by design. The IMP verifes *J* to avoid
